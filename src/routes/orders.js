...
router.get('/', requireLogin, (req, res) => {
  const orders = db.prepare(`
    SELECT o.*, u.full_name as created_by_name,
      ...
      (SELECT SUM(num_ctns) FROM order_lines WHERE order_id = o.id) as total_ctns
    FROM orders o
...
router.get('/:id', requireLogin, (req, res) => {
  ...
  const groups = db.prepare('SELECT * FROM order_groups WHERE order_id = ? ORDER BY sort_order, id').all(order.id);
  groups.forEach(g => {
    g.items = db.prepare('SELECT * FROM order_lines WHERE group_id = ? ORDER BY sort_order, id').all(g.id);
  });
  order.groups = groups;
  res.json(order);
});
...
router.post('/', requireLogin, (req, res) => {
  ...
  if (!d.order_no) {
    ...
    const last = db.prepare(`SELECT order_no FROM orders WHERE order_no LIKE ? ORDER BY id DESC LIMIT 1`).get(`AZL-${year}-%`);
    let seq = 1;
    if (last) { const parts = last.order_no.split('-'); seq = parseInt(parts[parts.length-1]) + 1; }
    d.order_no = `AZL-${year}-${String(seq).padStart(3,'0')}`;
  }
  const result = db.prepare(`
    INSERT INTO orders (order_no, buyer_name, consignee_text, buyer_text,
    ...
    VALUES (@order_no, @buyer_name, @consignee_text, @buyer_text,
      @country_destination, @invoice_no, @invoice_date, @email_dated,
      @order_date, @delivery_date, @port_loading, @port_discharge,
      @final_destination, @delivery_terms, @payment_terms, @payment_method,
      @currency, @partial_shipment, @transhipment, @delivery_note,
      @qty_variation_note, @status, @created_by)
  `).run({ ...d, created_by: req.session.userId, status: d.status || 'draft' });
  const orderId = result.lastInsertRowid;
  if (d.groups) saveGroupsAndLines(orderId, d.groups);
...
router.put('/:id', requireLogin, (req, res) => {
  ...
  db.prepare(`
    UPDATE orders SET buyer_name=@buyer_name, consignee_text=@consignee_text, buyer_text=@buyer_text,
...
router.delete('/:id', requireLogin, (req, res) => {
  ...
  if (order.status !== 'draft' && req.session.role !== 'admin')
    return res.status(403).json({ error: 'Only draft orders can be deleted' });
...
function saveGroupsAndLines(orderId, groups) {
  groups.forEach((grp, gi) => {
    const groupRes = db.prepare(`INSERT INTO order_groups (order_id, name, sort_order) VALUES (?,?,?)`).run(orderId, grp.name||'', gi);
    const groupId = groupRes.lastInsertRowid;
    (grp.items||[]).forEach((item, li) => {
      db.prepare(`
        INSERT INTO order_lines (group_id, order_id, description, product, size,
          pcs_per_ctn, num_ctns, packing_detail, hs_codes, barcode,
      ...
      `).run(
        ...
        item.product||'',
        item.size||'',
        parseFloat(item.pcs_per_ctn)||0,
        parseInt(item.num_ctns)||0,
        item.packing_detail||'',
        item.hs_codes||'',
        item.barcode||'',
